#!/usr/bin/clisp
;
; Conversion from French Revolutionary to Gregorian using the astronomical rule (equinox)
; Conversion du calendrier républicain vers le calendrier grégorien avec la règle astronomique des équinoxes
;
...
(let ((year  (parse-integer (car   *args*) ) )
    ...
    (let ((fixed  (cc3:fixed-from-french (cc3:french-date year month day1))))
         (print (cc3:gregorian-from-fixed fixed))
))
